...
(function() {
	...
	/**
		...
		@param {string | ARCameraParam}  cameraPara The ARCameraParam to use for image processing. If this is a string, the ARController treats it as an URL and tries to load it as a ARCameraParam definition file, calling ARController#onload on success. 
	*/
	...
	*/
	ARController.prototype.process = async function(image) {
        ...
        try{
            await arxjs.detectMarker(image);

            this.trackables.forEach(function(trackableId) {
                var transformation = arxjs.queryTrackableVisibilityAndTransformation(trackableId);
                if(transformation) {
                    ARController.transMatToGLMat(transformation, this.transform_mat);
            ...
            console.error("Unable to detect marker");
        }
	};
	...
	*/
     ...
     *              {
     *                  trackableType:  {string} 'barcode' / 'multi' / 'single' / '2D'
    ...
    ARController.prototype.addTrackable = async function(trackableObj) {
        ...
        if(trackableObj.trackableType === 'single') {
            loadTrackable(trackableObj.url, function(fileName) {
                var trackableId = arxjs.addTrackable(`single;${fileName};${trackableObj.width}`);
                this.trackables.push(trackableId);
                resolve(trackableId);
        ...
        // if(trackableObj.trackableType === 'multi') {
        //     loadMultiMarker(trackableObj.url, function() {
        //         // TODO
        //     });
        // }
    }
	...
	// */
	// ARController.prototype.loadTrackable = function(markerURL, onSuccess, onError) {
    //     if(markerURL){
    //         artoolkitX.addTrackable(this.id, markerURL, onSuccess, onError);
    //     }
    //     else {
    //         if(onError){
	 ...
	 */
	ARController.prototype.getTransMatSquare = function(markerUID, dst) {
        var dst = arxjs.arwQueryTrackableVisibilityAndTransformation(markerUID);
	...
	*/
	ARController.prototype.detectMarker = function(image) {
		if (this._copyImageToHeap(image)) {
			return arxjs.detectMarker(this.id);
		}
		return -99;
	};
	...
	*/
	...
	ARController.prototype.getCameraMatrix = function(nearPlane, farPlane) {
		return arxjs.arwGetProjectionMatrix(nearPlane, farPlane);
	};
	...
	*/
	var ARCameraParam = function(src, onload, onerror) {
		...
		if (src) {
            ...
            this.onerror("No camera parameter file defined!");
        }
	};
	...
	*/
	ARCameraParam.prototype._load = function(src) {
		if (this._src !== '') {
	...
	*/
	...
	var marker_count = 0;
	function loadTrackable(url, callback, onError) {
		var filename = '/trackable_' + marker_count++;
		_ajax(url, filename, function() {
	...
	function loadMultiMarker(url, callback, onError) {
		var filename = '/multi_marker_' + multi_marker_count++;
		_ajax(url, filename, function(bytes) {
	...
	function loadCamera(url, callback, errorCallback) {
		...
		if (typeof url === 'object') { // Maybe it's a byte array
			...
			_ajax(url, filename, writeCallback, errorCallback);
		}
	}
	...
	function _ajax(url, target, callback, errorCallback) {
